...
let is_closure = function E.Abs (_, _, Abs (_, _, e)) -> is_elementary e | _ -> false

let rec is_in_closure_form e =
  is_elementary e
  || match e with E.Let (_, e0, e1) -> is_closure e0 && is_in_closure_form e1 | _ -> false
...
let convert e =
  let expr_of_transformed (l, expr) =
    List.fold_right (fun (v, e) exp -> E.Let (v, e, exp)) l expr
  in
  expr_of_transformed (transform VarSet.empty e)
